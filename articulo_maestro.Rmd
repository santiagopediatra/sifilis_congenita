...
sensibilidades mantuvieron la dirección y la interpretación. Entre 250 casos con
datos completos, la OR de control prenatal adecuado para origen extranjero fue
...
registrada, con resultados robustos a la exclusión de 2021 y a priors razonables.
El análisis entre casos no permite evaluar mediación por control prenatal.
...
Se repitió el modelo excluyendo ambos estratos de 2021. La sensibilidad de priors
varió separadamente la desviación estándar del prior de origen (0,5 y 1,5) y la
tasa del prior exponencial de `shape` (0,5 y 2). El análisis logístico de control
...
`r val(ppc,"2021 mediana predictiva")` (ICr95% `r val(ppc,"2021 ICr95 inf")`–
`r val(ppc,"2021 ICr95 sup")`) y P(Yrep≥observado)=
`r fmt(val(ppc,"P(yrep >= observado)"),3)`. El valor observado se situó en la
cola superior de la distribución predictiva (percentil
...
frecuente bajo una tendencia suave.
...
`r fmt(s21$ICr95_inf[2])`–`r fmt(s21$ICr95_sup[2])`). En las sensibilidades de
priors, las medianas de IRR variaron entre `r fmt(min(spr$IRR))` y
...
```{r fig3, fig.cap="Figura 3. Control prenatal por origen materno entre los casos."}
...
y la interpretación se conservaron al excluir 2021 y bajo cambios razonables de
los priors, mientras que los diagnósticos MCMC y LOO no indicaron dependencia
crítica de problemas de muestreo o de un único estrato.
...
dentro de su ICr95%. Este hallazgo debe interpretarse en el contexto de posibles
cambios temporales de vigilancia, cobertura institucional, movilidad y atención,
ninguno de los cuales fue medido de forma suficiente para atribuir mecanismos.
...
grupo, posible dependencia temporal, cambios de cobertura —incluida posible
incompletitud de 2024—, covariables individuales solo entre casos y el supuesto
de una función temporal común con una IRR de origen constante.
...
exclusión de 2021 y a priors razonables. Los resultados no establecen causalidad
ni permiten evaluar si el control prenatal media la asociación observada.
...
```{r tablaS6}
ft(ppc|>mutate(valor=round(valor,4)),"Tabla S6. Resumen de comprobaciones predictivas posteriores","Yrep: conteos generados con posterior_predict().")
```
