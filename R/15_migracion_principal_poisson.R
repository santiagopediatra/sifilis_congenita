...
dir.create("figures/articulo/poisson", recursive = TRUE, showWarnings = FALSE)
...
ggsave("figures/articulo/poisson/figura1_tasas_modelo_poisson.png", figura1,
       width = 9, height = 5.5, dpi = 600, bg = "white")
ggsave("figures/articulo/poisson/figura1_tasas_modelo_poisson.pdf", figura1,
...
ggsave("figures/articulo/poisson/figura_s1_ppc_poisson.png", figura_s1,
       width = 8, height = 7, dpi = 600, bg = "white")
ggsave("figures/articulo/poisson/figura_s1_ppc_poisson.pdf", figura_s1,
...
archivos_codigo <- c(list.files("R", pattern = "\\.[Rr]$", full.names = TRUE),
...
referencias_nb <- map_dfr(archivos_codigo, function(a) {
  ...
  mutate(
    clasificacion = case_when(
      archivo == "R/15_migracion_principal_poisson.R" ~ "Sensibilidad de familia autorizada",
...
nuevos <- sort(c(
  ...
  list.files("figures/articulo/poisson", full.names = TRUE),
  "R/15_migracion_principal_poisson.R"
))
...
conservados <- c(
  ...
  "R/10_modelo_final_nb_spline_densidad.R",
  "R/11_generar_material_articulo.R",
  "R/12_cierre_auditoria_articulo.R",
  "R/13_sensibilidad_2024.R",
  "articulo_maestro.Rmd",
  "resultados_finales_auditados.Rmd",
  "RESULTADOS_FINALES_AUDITADOS.docx"
)
